...
namespace MobileBank.Forms
{
    public partial class ArmyHelp : Form
    {
        ...
        private void ArmyHelp_Load(object sender, EventArgs e)
        {
            ...
            ComboBoxArmyPayment.DataSource = operators;
            ComboBoxArmyPayment.ValueMember = "id_service";
            ComboBoxArmyPayment.DisplayMember = "serviceName";
        ...
        private void PaymentButton_Click(object sender, EventArgs e)
        {
            ...
            if (cardCurrency != "BYN")
            {
                MessageBox.Show("Помощь армии доступна только в белорусских рублях", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
            }
            ...
            if (!error)
            {
                ...
                if (DataStorage.attempts > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Помощь армии', '{ComboBoxArmyPayment.GetItemText(ComboBoxArmyPayment.SelectedItem)}','{transactionDate}','{transactionNumber}','{sum.ToString(CultureInfo.InvariantCulture)}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum.ToString(CultureInfo.InvariantCulture)}' where serviceName = '{ComboBoxArmyPayment.GetItemText(ComboBoxArmyPayment.SelectedItem)}' and serviceType = 'Charity'";
                    var queryTransaction4 = $"insert into clientPersonalAccount(personal_account,id_service,id_client) values('{PersonalAccountTextBox.Text}',(select id_service from clientServices where serviceName = '{ComboBoxArmyPayment.GetItemText(ComboBoxArmyPayment.SelectedItem)}'),'{DataStorage.idClient}')";
