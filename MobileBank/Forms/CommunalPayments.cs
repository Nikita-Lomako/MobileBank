...
namespace MobileBank.Forms
{
    public partial class CommunalPayments : Form
    {
        ...
        private void PaymentButton_Click(object sender, EventArgs e)
        {
            ...
            if (cardCurrency != "BYN")
            {
                MessageBox.Show("Пополнение коммунальных услуг можно проводить только в белорусских рублях", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
            }
            ...
            if (!error)
            {
                ...
                if (DataStorage.attempts > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Оплата коммунальных услуг','{ComboBoxCommunalPayment.GetItemText(ComboBoxCommunalPayment.SelectedItem)}','{transactionDate}','{transactionNumber}','{sum.ToString(CultureInfo.InvariantCulture)}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum.ToString(CultureInfo.InvariantCulture)}' where serviceName = '{ComboBoxCommunalPayment.GetItemText(ComboBoxCommunalPayment.SelectedItem)}' and serviceType = 'Communal'";
                    var queryTransaction4 = $"insert into clientPersonalAccount(personal_account,id_service,id_client) values('{PersonalAccountTextBox.Text}',(select id_service from clientServices where serviceName = '{ComboBoxCommunalPayment.GetItemText(ComboBoxCommunalPayment.SelectedItem)}'),'{DataStorage.idClient}')";
